...
function GetUniqueItemDiv(el, itemType) {
  ...
  if (el.Equipment.EquipmentType == 0) {
    // Armor type
    if (el.Equipment.DamageString) {
      var $dmgSpan = $("<span>");
      $dmgSpan.addClass("col-sm-12");
      $dmgSpan.text(
        el.Equipment.DamageStringPrefix + ": " + el.Equipment.DamageString
      );
      $item.append($dmgSpan);
    }
  }
...
function RenderRunewords() {
  ...
  for (var i = 0; i < Runewords.length; i++) {
    ...
    for (var j = 0; j < el.Runes.length; j++) {
      var rune = el.Runes[j];
      var $spanRune = $("<span>")
        .text(rune.Name.replace(" Rune", ""))
        .addClass("unique-text");
...
function RenderSets() {
  ...
  for (var i = 0; i < Sets.length; i++) {
    ...
    for (var j = 0; j < set.PartialProperties.length; j++) {
      ...
      $propSpan.addClass("col-sm-12");
      $propSpan.text(
        prop.PropertyString + " (" + Math.floor(prop.Index / 2 + 2) + " Items)"
      );
...
function GetUniqueSearch() {
  ...
  for (var i = 0; i < AllUniques.length; i++) {
    ...
    // Equipment Types
    if (!ArrayContainsString(equipmentTypes, el.Equipment.Name)) {
...
function GenerateSearchBarUniques() {
  ...
  equipmentNameDropdown.html(
    "<option disabled selected>Equipment Name</option>"
  );
...
function Search(forceSearch = false) {
  ...
  if (activeField === "nav-uniques") {
    ...
    for (var i = 0; i < AllUniques.length; i++) {
      ...
      if (
        (typeSearch === null ||
          typeSearch.toLowerCase() === "any" ||
          el.Equipment.Type.Name === typeSearch) &&
        (equipmentNameSearch === null ||
          equipmentNameSearch.toLowerCase() === "any" ||
          el.Equipment.Name === equipmentNameSearch)
      ) {
        if (text.length > 1) {
          ...
          for (var j = 0; j < el.Properties.length; j++) {
            var prop = el.Properties[j];
            if (
              prop.PropertyString.toLowerCase().indexOf(text.toLowerCase()) > -1
            ) {
              if (!ArrayContainsName(searchedUniques, el.Name)) {
                searchedUniques.push(el);
              }
    ...
    for (var i = 0; i < AllRunewords.length; i++) {
      ...
      if (
        typeSearch === null ||
        typeSearch.toLowerCase() === "any" ||
        ArrayContainsName(el.Types, typeSearch)
      ) {
        if (text.length > 1) {
          if (el.Name.toLowerCase().indexOf(text.toLowerCase()) > -1) {
    ...
    for (var i = 0; i < AllCubeRecipes.length; i++) {
      ...
      if (
        el.CubeRecipeDescription.toLowerCase().indexOf(text.toLowerCase()) > -1
      ) {
        CubeRecipes.push(el);
      }
    }
      ...
      if (
        ...
        if (text.length > 1) {
          ...
          for (var j = 0; j < el.PartialProperties.length; j++) {
            ...
            if (
              property.PropertyString.toLowerCase().indexOf(
                text.toLowerCase()
              ) > -1
            ) {
              if (!ArrayContainsName(searchedSets, el.Name)) {
...
function CompareItems(arr1, arr2) {
  ...
  for (var i = arr1.length; i--; ) {
    if (arr1[i].Name.toLowerCase() !== arr2[i].Name.toLowerCase()) return false;
  }
  return true;
}
